...
export default function(piece, compboard = board) {
  ...
  switch (piece.constructor) {
    ...
    case Bishop:
      //check corners
      if ([y_ax, x_ax] == [0, 0]) possibleOffsets = possibleOffsets.filter((move) => move[0] > 0 || move[1] > 0); else
      if ([y_ax, x_ax] == [0, 7]) possibleOffsets = possibleOffsets.filter((move) => move[0] < 0 || move[1] > 0); else
      if ([y_ax, x_ax] == [7, 0]) possibleOffsets = possibleOffsets.filter((move) => move[0] > 0 || move[1] < 0); else
      if ([y_ax, x_ax] == [7, 7]) possibleOffsets = possibleOffsets.filter((move) => move[0] > 0 || move[0] > 0);
      ...
      if ([y_ax, x_ax] != [0, 0]) {
        ...
        for (let i = 1; i <= x_ax && i <= y_ax && found === false; i++) {
          if (compboard[y_ax - i][x_ax - i]?.piece !== null && compboard[y_ax - i][x_ax - i]?.piece !== undefined ) found = true;
          distance = i;
        }
        
        possibleOffsets = possibleOffsets.filter((move) => {
          if (-move[0] >= distance && -move[1] <= distance) return move; else
          if (move[0] > 0 || move[1] > 0) return move;
        });
      }
