...
export function set_FEN(FEN_string) {
  ...
  piece_str.split("/").forEach((row, py) => {
    ...
    Array.from(row).forEach((i, px) => {
      ...
      if (
        ['k', 'r'].includes(cpiece.symbol.toLowerCase()) &&
        !coordCompare([y, x], cpiece.defaultPos)
      ) {
        compboard[y][x].piece.hasMoved = true;
      }
  ...
  if (
    ...
    (!castles.includes("K") &&
      !castles.includes("Q") &&
      compboard[0][4].piece?.symbol.toLowerCase() === 'k')
  ) {
    compboard[0][4].piece.hasMoved = true;
  }
  if (!castles.includes("K") && compboard[0][7].piece?.symbol.toLowerCase() === 'r') {
    compboard[0][7].piece.hasMoved = true;
  }
  if (!castles.includes("Q") && compboard[0][0].piece?.symbol.toLowerCase() === 'r') {
  ...
  if (
    ...
    (!castles.includes("k") &&
      !castles.includes("q") &&
      compboard[7][4].piece?.symbol.toLowerCase() === 'k')
  ) {
    compboard[7][4].piece.hasMoved = true;
  }
  if (!castles.includes("k") && compboard[7][7].piece?.symbol.toLowerCase() === 'r') {
    compboard[7][7].piece.hasMoved = true;
  }
  if (!castles.includes("q") && compboard[7][0].piece?.symbol.toLowerCase() === 'r') {
...
export function get_FEN() {
  ...
  if (lKing?.symbol.toLowerCase() === 'k' && !lKing.hasMoved) {
    if (lKingsRook?.symbol.toLowerCase() === 'r' && !lKingsRook.hasMoved) {
      castles.push("K");
    }
    if (lQueensRook?.symbol.toLowerCase() === 'r' && !lQueensRook.hasMoved) {
      castles.push("Q");
    }
  }
  ...
  if (dKing?.symbol.toLowerCase() === 'k' && !dKing.hasMoved) {
    if (dKingsRook?.symbol.toLowerCase() === 'r' && !dKingsRook.hasMoved) {
      castles.push("k");
    }
    if (dQueensRook?.symbol.toLowerCase() === 'r' && !dQueensRook.hasMoved) {
      castles.push("q");
    }
  }
